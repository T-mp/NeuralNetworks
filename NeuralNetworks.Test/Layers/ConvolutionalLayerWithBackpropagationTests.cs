...
public class ConvolutionalLayerWithBackpropagationTests
{
    ...
    public void TrainSimpleConvolutionalLayer()
    {
        ...
        float[] target = [2f, 3f, 4f, 5f, 6f];

        // Lernrate
        float learningRate = 0.001f;//0.01f;

        // ConvolutionalLayer initialisieren (FilterSize = 4, Stride = 1, UseBias = true)
        var convLayer = new ConvolutionalLayerWithBackpropagation(filterSize: 4, stride: 1, useBias: true, NN.Initializers.GlorotUniform(new TestRandomProvider(21)), NN.Initializers.Zeros());
    ...
    public void ForwardBackward_ShouldReduceLoss_OnSimplePattern()
    {
        ...
        var layer = new ConvolutionalLayerWithBackpropagation(filterSize: 3, stride: 1, useBias: true, NN.Initializers.GlorotUniform(new TestRandomProvider(21)), NN.Initializers.Zeros());
