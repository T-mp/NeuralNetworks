...
public class RecurrentLayerWithBackpropagationTests
{
    ...
    public void Backward_UpdatesWeightsAndBiases()
    {
        ...
        //Achtung der Test kann erst ab dem zweiten Aufruf von Update() funktionieren, da sonst die preNodeValues noch nicht gesetzt sind.
        // Erste Runde, um die Layer-Parameter zu initialisieren
        layer.Update(input);
        layer.Backward(error, 0.01f);

        // zweite Runde und anschließend der Test
        layer.Update(input);

        var weightsBefore = (float[,])layer.Parameters.Get2dVectorCopy("weights");
        var recurrentBefore = (float[])layer.Parameters.Get1dVector("recurrentWeights").Clone();
        var biasesBefore = (float[])layer.Parameters.Get1dVector("biases").Clone();
