...
public class GruLayer : IModelLayer
{
    ...
    private void UpdateCell(int index, ReadOnlySpan<float> inputs)
    {
        ...
        BackpropagationStateSet("forgetGateInputs", index, forgetGateInput);
        BackpropagationStateSet("forgetGates", index, forgetGate);
        BackpropagationStateSet("candidateInputs", index, candidateInput);
        BackpropagationStateSet("candidates", index, candidate);
    }

    protected float Mutliply(float[,] weights, ReadOnlySpan<float> inputs, int weightsIndex)
    {
        var result = 0f;
