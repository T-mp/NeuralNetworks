...
public class Pooling2dLayer : IModelLayer
{
        ...
        for (int fx = 0; fx < WindowSize.Width; fx += 1)
        {
            for (int fy = 0; fy < WindowSize.Height; fy += 1)
            {
                ...
                if (inputValue > nodeValue)
                {
                    ...
                    BackpropagationStateSet("minMaxIds", nodeX, nodeY, inputX * InputSize.Width + inputY);
                }
            }
        }
    ...
    protected virtual void BackpropagationStateSet(string name, int index1, int index2, float value) { }
}
